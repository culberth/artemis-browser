...
class QueueControllerTest
{
    ...
    void refusesUnknownQueueName() throws Exception
    {
        ...
        mockMvc.perform(get("/queues").param("name", "secrets").header("Host", "localhost")).andExpect(status().isOk())
                .andExpect(model().attributeExists("error")).andExpect(model().attributeDoesNotExist("browse"));
    ...
    {
        QueueStats stats = new QueueStats("orders", "orders.address", "MULTICAST", 3, 0, 0, 1, 10, 7, true, false);
        ...
        mockMvc.perform(get("/queues").param("name", "orders").header("Host", "localhost")).andExpect(status().isOk())
    ...
    {
        QueueStats stats = new QueueStats("orders", "orders", "ANYCAST", 0, 0, 0, 0, 0, 0, true, false);
        given(queueDirectory.stats("orders")).willReturn(stats);
        given(browseService.browse(anyString(), anyString(), anyInt()))
                .willReturn(new BrowseResult("orders", 500, false, List.of()));

        mockMvc.perform(get("/queues").param("name", "orders").param("limit", "100000").header("Host", "localhost"))
