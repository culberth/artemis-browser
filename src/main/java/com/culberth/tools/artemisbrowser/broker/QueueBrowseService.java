 ...
 *
 * <p>
 * This uses a JMS {@link QueueBrowser}, which is the one read path the JMS spec guarantees is non-destructive: it hands
 * out copies and never acknowledges, so nothing is consumed, nothing is marked delivered, and no redelivery counter
 * moves. A {@code MessageConsumer} — even one that never acknowledges — would put messages into the delivering state
 * and is not an option here.
 */
@Service
public class QueueBrowseService
{
    ...
    private MessageSummary summarise(Message message, int position) throws JMSException
    {
        ...
        return new MessageSummary(position, message.getJMSMessageID(), message.getJMSCorrelationID(), typeOf(message),
                message.getJMSTimestamp() == 0 ? null : message.getJMSTimestamp(),
                formatTimestamp(message.getJMSTimestamp()), message.getJMSPriority(),
                message.getJMSDeliveryMode() == jakarta.jms.DeliveryMode.PERSISTENT, message.getJMSRedelivered(),
                message.getJMSExpiration() == 0 ? null : message.getJMSExpiration(),
                message.getStringProperty("JMSXGroupID"), truncated ? body.substring(0, bodyPreviewChars) : body,
