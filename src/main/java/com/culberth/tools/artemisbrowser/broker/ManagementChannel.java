 ...
 *
 * <p>Queue listing is not a JMS operation — the JMS API has no notion of "what queues exist". It is
 * a management query: a message sent to {@code activemq.management} naming a resource and an
 * operation, answered on a reply queue. Doing it this way rather than over JMX means no extra port
 * and no Jolokia; the broker user just needs {@code manage} permission on that address.
 *
 * <p>The producer, consumer and temporary reply queue are created once and reused. Every method is
 * synchronized because a JMS {@link Session} is not thread-safe and this one is shared by whatever
 * requests the user's HTTP session makes.
 */
public class ManagementChannel implements AutoCloseable {
     ...
     *
     * <p>The broker counts it as a queue like any other, so without excluding it the tool lists
     * its own plumbing as a browsable queue — with a UUID for a name and a different one each
     * time the user reconnects.
     */
    ...
    private Object exchange(String what, RequestBuilder builder) {
        try {
            ...
            if (reply == null) {
                throw new BrokerException("The broker did not answer " + what + " within "
                        + timeoutMillis + "ms. It may be under load, or the management address may be"
                        + " disabled.");
            }
